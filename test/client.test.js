...
const createNock = () => {
  return nock('https://airtime-api.dtone.com')
...
test('sendRequest - should throw TransferToError when tt error_code is not 0', t => {
  ...
  client.sendRequest({ foo: 'foo' }).catch(e => {
...
test('sendRequest - should send its args as query params', t => {
  ...
  client.sendRequest({ action, info_type: infoType }).then(_ => t.end());
});
...
test('sendRequest - should parse comma separated values to json array', t => {
  ...
  client.sendRequest({ bar: 'bar' }).then(data => {
    t.ok(data.foo);
...
test('getCountries', t => {
  ...
  client.getCountries().then(_ => t.end());
});
...
test('getCountry', t => {
  ...
  client.getCountry().then(_ => t.end());
});
...
test('getOperator', t => {
  ...
  client.getOperator(operatorId).then(_ => t.end());
});
...
test('getMsisdnInfo - with no options', t => {
  ...
  client.getMsisdnInfo(msisdn).then(_ => t.end());
});
...
test('getMsisdnInfo - with options', t => {
  ...
  client.getMsisdnInfo(msisdn, options).then(_ => t.end());
});
...
test('topup', t => {
  ...
  client.topup(topupData).then(_ => t.end());
});
